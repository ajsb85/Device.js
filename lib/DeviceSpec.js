...
// --- define ----------------------------------------------
// --- interface -------------------------------------------
function DeviceSpec() { // @help: DeviceSpec
}
...
DeviceSpec.token  = getDeviceToken; // DeviceSpec.token(userAgent:String = navigator.userAgent):String

// --- implement -------------------------------------------
function detect(device) { // @arg String(= ""): device token.
                          // @ret Object: { OS, SOC, CPU, GPU, RAM, BRAND, DEVICE, DISPLAY, TOUCH }
                          //     OS        - Object: { BEGIN: String, END: String }
                          ...
                          //        .TYPE  - Number(= 0): CPU base clock. zero in no-data. eg: 1.7
                          //        .CORES - Integer(= 0): CPU Cores. zero in no-data. eg: 2 (dual core)
                          //     GPU       - Object: { TYPE: String, NAME: String }
                          //        .TYPE  - String(= ""): GPU type. "" in no-data. eg: "Adreno"
                          //        .NAME  - String(= ""): GPU name. "" in no-data. eg: "Adreno 330"
                          //     RAM       - Integer: RAM size (unit: GB).
                          //     BRAND     - String: Brand name. eg: "GOOGLE", "SONY"
                          //     DEVICE    - String: device name(device token). eg: "Nexus 7"
                          //     DISPLAY   - Object: { PPI: Number, DPR: Number, LONG: Integer, SHORT: Integer, TOUCH: Integer }
                          //        .PPI   - Integer(= 0): display pixel per inch. zero in no-data.
                          //        .DPR   - Integer(= 0): device pixel ratio. zero in no-data.
                          //        .LONG  - Integer(= 0): display SHORT, or short edge. zero is no-data.
                          //        .SHORT - Integer(= 0): display SHORT, or long edge. zero is no-data.
                          //        .TOUCH - Integer: Touch points.
                          // @desc: detect mobile device specs.
                          // @help: DeviceSpec.detect
    ...
    var result = {
            ...
            BRAND:  "",
            DEVICE: "",
            DISPLAY:{ PPI: 0, DPR: 0, SHORT: 0, LONG: 0, TOUCH_POINTS: 0 }
        };
    ...
    if (device in DEVICE_CATALOG) {
        ...
        result.RAM            = spec[8];
        result.DISPLAY.TOUCH_POINTS = spec[9];
...
//               BRAND     SOC      BEGIN,END  SHORT,LONG   PPI   DPR   RAM   TOUCH
    ...
    "iPad mini Retina":
                [APPLE,    A7,        700,0,    1536,2048,  326,    2,    1],
