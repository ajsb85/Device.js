...
Device["query"] = Device_query;     // Device.query(selector:DeviceQueryString):IDArray
...
function Device_query(selector,        // @arg DeviceQueryString: query string. "GPU.TYPE=Adreno;GPU.ID=330"
                      caseSensitive) { // @arg Boolean(= false): true is case-sensitive, false is ignore case.
                                       // @ret IDArray: device-id or soc-id array. [ID, ...]
                                       // @help: Device.query
                                       // @desc: Query device catlog.
//{@assert
    _if(typeof selector !== "string", "invalid Device.query(selector,): " + selector);
    if (caseSensitive !== undefined) {
        _if(typeof caseSensitive !== "boolean", "invalid Device.query(, caseSensitive)");
    }
//}@assert
    ...
    var map = {
            ...
            "OS.VERSION.PRE":       [DEVICE_CATALOG, 3, function(v) { return _version(v).valueOf() }],
            "OS.VERSION.HIGHEST":   [DEVICE_CATALOG, 4, function(v) { return _version(v).valueOf() }],
...
function _detectDeviceSpec(userAgent, // @arg UserAgentString:
    ...
    if (catalog) {
        ...
        var socCatalog = SOC_CATALOG[ catalog[2] ];

        spec["DEVICE"]["ID"]             = device.id;
        spec["DEVICE"]["MAYBE"]          = device.maybe;
        spec["DEVICE"]["BRAND"]          = catalog[1];
        spec["DEVICE"]["SOC"]            = catalog[2];
        spec["DEVICE"]["GPS"]            = /GPS/.test(catalog[11]);
        spec["OS"]["VERSION"]["PRE"]     = _version(catalog[3]);
        spec["OS"]["VERSION"]["HIGHEST"] = _version(catalog[4]);
        ...
        spec["INPUT"]["TOUCH"]           = !!catalog[10]; // to Boolean value
        spec["INPUT"]["TOUCHES"]         = catalog[10];
        spec["MEMORY"]["RAM"]            = catalog[9];
        spec["DISPLAY"]["PPI"]           = catalog[7];
        spec["DISPLAY"]["DPR"]           = catalog[8];
        spec["DISPLAY"]["LONG"]          = Math.max(catalog[5], catalog[6]);
        spec["DISPLAY"]["SHORT"]         = Math.min(catalog[5], catalog[6]);
        spec["NETWORK"]["3G"]            = /3G/.test(catalog[11]);
        spec["NETWORK"]["LTE"]           = /LTE/.test(catalog[11]);
        spec["NETWORK"]["NFC"]           = /NFC/.test(catalog[11]);
        spec["NETWORK"]["WIFI"]          = /WIFI/.test(catalog[11]);
    }
    spec["OS"]["TYPE"]               = os;
...
function _getWindowsPhoneDevice(userAgent, // @arg String:
    ...
    return { id: id, maybe: false, ieVersion: ieVersion };
}
