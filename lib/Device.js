...
//}@assert
    ...
    var spec = Device_spec( Device_token(userAgent) );

    // Set current OS version.
    spec["OS"]["VERSION"]["CURRENT"] = Device_getOSVersion(userAgent);
...
function Device_spec(token) { // @arg DeviceTokenString: device token.
                              ...
                              //           VERSION: {
                              //             CURRENT: Integer(= 0): Current version.
                              //             FIRST:   Integer(= 0): First release version.
                              //             LAST:    Integer(= 0): Last release version.
                              //           }
                              //        },
                              //        CPU: {
...
//}@assert
    ...
    var result = {
            ...
            "OS":       { "TYPE": "", "VERSION": { "CURRENT": 0, "FIRST": 0, "LAST": 0 } },
    ...
    // OS.VERSION accessor
    result["OS"]["VERSION"]["valueOf"] = function() { return this.CURRENT | 0; };
    ...
    if (spec) {
        ...
        result["OS"]["VERSION"]["CURRENT"] = _version(0);
        result["OS"]["VERSION"]["FIRST"] = _version(spec[2]);
        result["OS"]["VERSION"]["LAST"]  = _version(spec[3]);
...
function Device_getOSVersion(userAgent) { // @arg UserAgentString:
                                          // @ret DeviceVersionNumberObject: { VERSION, MAJOR, MINOR, PATCH }
                                          //          VERSION - Number(= 0.0): OS Major.Minor version
                                          //          MAJOR - Integer(= 0): OS Major version
                                          //          MINOR - Integer(= 0): OS Minor version
                                          //          PATCH - Integer(= 0): OS Patch version
                                          // @help: Device.getOSVersion
    ...
    return { valueOf: function() { return parseFloat(this.MAJOR + "." + this.MINOR); },
             MAJOR: major, MINOR: minor, PATCH: patch };
}
