...
//}@assert
    ...
    return _detect( userAgent || (global["navigator"] || {})["userAgent"] || "" );
}
...
function _newSpecObject(userAgent) { // @arg UserAgentString:
                                     // @ret SpecObject: { DEVICE, OS, CPU, GPU, INPUT,
                                     //                    MEMORY, DISPLAY, NETWORK, BROWSER }
    return {
        "DEVICE": {
        ...
        "MEMORY": {
            ...
            "ROM":   0.0        // Flash ROM size (unit: GB). [[RESERVED]]
        },
        "DISPLAY": {
...
function _detect(userAgent) { // @arg UserAgentString:
    ...
    var droid = /Android/.test(userAgent);
    var token = droid ? _getAndroidDeviceToken(userAgent)
              : ios   ? _getiOSDeviceToken(userAgent)
                      : "";
    var spec = _newSpecObject(userAgent);
    var device = ANDROID_DEVICE[token] || IOS_DEVICE[token] || null;

    if (device) {
        if (device["hook"]) { // has hook function.
            token = device["hook"](token); // overwrite device token. "Nexus 7" -> "Nexus 7 (2013)"
            device = ANDROID_DEVICE[token] || IOS_DEVICE[token]; // re-search
        }
        var soc = SOC_CATALOG[ device[1] ];

        spec["DEVICE"]["TOKEN"]        = token;
        spec["DEVICE"]["BRAND"]        = device[0];
        spec["DEVICE"]["SOC"]          = device[1];
        spec["DEVICE"]["GPS"]          = /G/.test(device[10]);
        spec["OS"]["TYPE"]             = droid ? "Android" :
                                         ios   ? "iOS" : "";
        spec["OS"]["VERSION"]["CURRENT"] = _version(_current(userAgent));
        spec["OS"]["VERSION"]["FIRST"] = _version(device[2]);
        spec["OS"]["VERSION"]["LAST"]  = _version(device[3]);
        spec["CPU"]["TYPE"]            = soc[0];
        spec["CPU"]["CLOCK"]           = soc[1];
        spec["CPU"]["CORES"]           = soc[2];
        spec["CPU"]["SIMD"]            = soc[3] !== "Tegra2"; // Tegra2 NEON disabled.
        spec["GPU"]["TYPE"]            = soc[3];
        spec["GPU"]["ID"]              = soc[4];
        spec["INPUT"]["TOUCH"]         = !!device[9]; // to Boolean value
        spec["INPUT"]["TOUCHES"]       = device[9];
        spec["MEMORY"]["RAM"]          = device[8];
      //spec["MEMORY"]["ROM"]          = 0;
        spec["DISPLAY"]["PPI"]         = device[6];
        spec["DISPLAY"]["DPR"]         = device[7];
        spec["DISPLAY"]["LONG"]        = Math.max(device[4], device[5]);
        spec["DISPLAY"]["SHORT"]       = Math.min(device[4], device[5]);
        spec["NETWORK"]["3G"]          = /3G/.test(device[10]);
        spec["NETWORK"]["LTE"]         = /LTE/.test(device[10]);
        spec["NETWORK"]["NFC"]         = /NFC/.test(device[10]);
        spec["NETWORK"]["WIFI"]        = /WIFI/.test(device[10]);
    }
    return spec;
}
...
//  var airPlay = !!global["WebKitPlaybackTargetAvailabilityEvent"]; // AirPlay API. iPhone4s++. iPad2++, iPad mini++, iPod touch 5++
    ...
    switch (deviceToken) {
    case "iPad":
        deviceToken = (devicePixelRatio === 1) ? "iPad 2"  // maybe, candidate: iPad 2, iPad mini
