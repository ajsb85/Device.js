...
//}@assert
    ...
    if (id in DEVICE_CATALOG) {
        return _createSpecObjectById(id);
    } else if (id in SOC_CATALOG) {
        var specObject = _createSpecObject();
...
function _createSpecObjectById(id) { // @arg String: device-id
...
function _detectDeviceSpec(userAgent, // @arg UserAgentString:
    ...
    var specObject = specData ? _createSpecObjectById(id)
                              : _createSpecObject();
